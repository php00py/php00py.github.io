---
title: Joomla 3.4.6 Remote Code Execution漏洞复现
tags:
  - 漏洞复现
categories:
  - 漏洞复现
copyright: true
abbrlink: 6759ae62
date: 2019-10-09 16:01:50
---
**0x00.Joomla介绍**  
Joomla!是一套全球知名的内容管理系统。 Joomla!是使用PHP语言加上MySQL数据库所开发的软件系统，目前最新版本是3.9.11
。可以在Linux、 Windows、MacOSX等各种不同的平台上执行。目前是由Open Source
Matter这个开放源码组织进行开发与支持，这个组织的成员来自全世界各地，小组成员约有150人，包含了开发者、设计者、系统管理者、文件撰写者，以及超过2万名的参与会员。  
...
在window下使用phpstudy搭建，将下载的压缩包解压到phpstudy的www目录下。然后输入http://127.0.0.1/Joomla
进行安装。然后按照提示进行配置。如图：  
![在这里插入图片描述](https://img-blog.csdnimg.cn/20191009153546794.png)
配置完成后，下一步，等待安装。  
![在这里插入图片描述](https://img-blog.csdnimg.cn/20191009153640226.png)
**0x03漏洞复现**

  1. 使用脚本进行验证：显示Vulnerable证明存在漏洞 

![在这里插入图片描述](https://img-blog.csdnimg.cn/20191009153851122.png)
...
3.漏洞利用  
在configuration.php中写入了一句话木马，nfewspvdwsxjjiioynptjdqdcpirdjljqbfgufhaaukotihzeo为密码  
![在这里插入图片描述](https://img-blog.csdnimg.cn/20191009154721853.png)
查看configuration.php，在文件末尾写入了一句话木马  
![在这里插入图片描述](https://img-blog.csdnimg.cn/20191009155719841.png)

使用菜刀连接，连接为http://127.0.0.1/Joomla//configuration.php  
![在这里插入图片描述](https://img-blog.csdnimg.cn/20191009155353843.png)
**0x04参考**  
